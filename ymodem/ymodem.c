...
static uint32_t crc32(const uint8_t* buf, unsigned long count) {
	...
	static const uint32_t crc_table[256] = { 0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F,
...
unsigned long ymodem_receive(uint8_t* buf, uint32_t length) {
								...
								if (packets_received == 0) {
									...
									if (i < PACKET_HEADER + 4) { /* filename packet has data */
										...
										if (size > length) {
											...
											_sleep(1);
#ifdef DEBUG
											printf("\nrcv buffer too small (0x%08"PRIX32" vs 0x%08"PRIX32")\n", length,size);
#endif
											errno = YMODEM_ERROR_BUFFERTOOSMALL;
											return 0;
										}
										_putchar(ACK);
									...
									if ((buf_ptr + packet_length) - buf > length) {
										...
										_sleep(1);
#ifdef DEBUG
										printf("\nbuffer overflow: %"PRId32"\n", length);
#endif
										errno = YMODEM_ERROR_BUFFEROVERFLOW;
										return 0;
									}
...
#endif
							errno = YMODEM_ERROR_TOOMANYRECVERRORS;
							return 0;
						}
					}
					_putchar(CRC);
	...
	if (size > 0) {
		printf("read:%s\n", file_name);
#ifdef WITH_CRC32
		printf("crc32:0x%08"PRIX32", len:0x%08"PRIX32"\n", crc32(buf, size), size);
#else
